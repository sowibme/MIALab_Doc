...
\begin{abstract}
As a practical group work to get used to medical image segmentation, we received an image segmentation pipeline which can segment MRI images into the classes grey matter, white matter and ventricles. The main goal of our work is to assess the influence of preprocessing, an early stage of the segmentation pipeline, on the segmentation performance. Applying different filters and transformation to the raw images, we conducted several experiments of different degrees of preprocessing of the data. The experiments have shown that filter performance is dependent on the target region and that bias field correction does not seem to have a big influence on the segmentation performance.
\end{abstract}
...
\caption{Example segmentation of a healthy brain T2 weighted image: On the left side is the unsegmented image, on the right side the segmentation with white being classified as ventricles, darkgrey the white matter and lightgray the grey matter.}
\label{fig:brainseg}
\end{figure}
...
\section{Results}
Figure \ref{fig:result} contains segmentation performances for all the conducted experiments. For the different experiments, the filters and preprocessing steps have been applied to the already bias field corrected, and skull stripped data, shown in Figure \ref{fig:pipeline-preprossesing}.  In the case that more than one filter or preprocessing step was applied, the names of the steps are mentioned in the executed order. For instance \textit{Zscore-MedianR2} refers to first applying Z score normalisation to the image, and then filtering it with a median filter with kernel size two. The calculation times for the different experiments varied a lot: The experiments where gaussian and median filtering was applied took about 1 hour to complete. In contrast, bilateral filtering almost tripled this calculation time, ending only after about three hours. As we also had an already skull stipped and bias field corrected dataset, we do not have the exact calculation times for these. We were however told that it took several hours for the bias field correction of the dataset.

\begin{figure}[!h]
\centering
\includegraphics[width=0.49\textwidth]{png/preprocessing-pipeline.png}
\caption{Computation order of preprocessing steps}
\label{fig:pipeline-preprossesing}
\end{figure}
