...
% The paper headers
\markboth{Medical Image Analysis Lab, december 2017}%
...
\begin{abstract}
As a practical group work to get used to medical image segmentation we received an image segmentation pipeline which can segment MRI images into the classes greymatter, whitematter and ventricles. The main goal of our work is to asses the influence of preprocessing, an early stage of the segmentation pipeline, on the segmentation performance. Applying different filters and transformation to the raw images we conducted several experiments of different degrees of preprocessing of the data. The experiments have shown that filter performance is depenedent on the target region and that bias field correction does not seem to have a big influence on the segmentation performance.
\end{abstract}
...
\section{Discussion}
One can see that the preprocessing step has indeed an influence on the segmentation output.
Experiment \textit{02Skull} compared to the reference calculation \textit{00raw} shows that the influence of skull stripping is the most significant of all our experiments. It increases all the dice coefficients and reduces the variances for the grey- and whitematter substantially. Conversely we were quite surprised that bias field correction (\textit{01biasfieldcorr}) has the oppisite effect: Although increasing the dice for the ventricles and greymatter a little bit, all the variances have been increased. With the fact in mind that bias field correction took several hours of computation, this does not seem to be a very efficient preprocessing step.

The median filter with kernel size three helps to increase the segmentation performance of the ventricles. But when looked at the white- and greymatter the median as well as the gaussian filter do not help improve the performance. That allows an assumption that in this part we haven't gaussian noise or outliers.

\section{Conclusion}
With good and approrpiate preprocessing the segmentation performance can be increased. The experiments however have shown that while the performance for a certain region, for instance the ventricles, may be increased with the use of a filter the contrary may be true to the other regions: The filters which help increasing the the ventricle segmentation reduce also the dices from the other class or the other way around. The impact of bias field correction was in our case not significant and we would consider to drop it in future experiments. Bilateral filtering did not lead to any significant increase in the segmentation performance. This is however most certainly due to a wrong paramenter setting. As the calculation with the bilateral filter took several hours, we only conducted one experiment with the default parameters. To have a better understanding of the performance we should run more experiments and try to tune the paramenters. While we have performed some expermients, there still exist a multitude if filters and techniques to be examined as a potential beneficial preprocessing step.
